...
__global__ void flip1_TRI(float *confx, float *confy, float *confz, unsigned int *rngState, float* Hs, float* invTs){
  ...
  r = (unsigned int)UINT_MAX*(float(int(1000000*(du+10))%1000)/1000.);//WarpStandard_Generate(rngRegs, rngShmem);
  sz = r * NORM - 1;
  th = asin( sz );
  r = (unsigned int)UINT_MAX*(float(int(1000000*(sz+1))%1000)/1000.);//WarpStandard_Generate(rngRegs, rngShmem);
  phi = r*TOPI;
  sz = sin( th );
  sx = cos( th ) * cos( phi );
  sy = cos( th ) * sin( phi );
