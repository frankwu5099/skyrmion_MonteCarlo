...
__global__ void flip1_TRI(float *confx, float *confy, float *confz, unsigned int *rngState, float* Hs, float* invTs, curandStateMtgp32 *state){
  ...
  if (du>0.0000001){
    ...
    confx[flip_coo(k, j, i)] = hx * costh + nx;
    confy[flip_coo(k, j, i)] = hy * costh + ny;
    confz[flip_coo(k, j, i)] = hz * costh + nz;
  }
  else{
    confx[flip_coo(k, j, i)] = sinth * sx;
    confy[flip_coo(k, j, i)] = sinth * sy;
    confz[flip_coo(k, j, i)] = hz * costh;
  }
  __syncthreads();
